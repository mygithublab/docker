...
RUN cd /tmp && wget -O nagioscore.tar.gz https://github.com/NagiosEnterprises/nagioscore/archive/nagios-4.3.4.tar.gz \
 && wget --no-check-certificate -O nagios-plugins.tar.gz https://github.com/nagios-plugins/nagios-plugins/archive/release-2.2.1.tar.gz \
 && tar xzvf nagioscore.tar.gz && cd /tmp/nagioscore-nagios-4.3.4/ && ./configure && make all \
 && useradd nagios && usermod -a -G nagios apache && make install && make install-init && chkconfig --add nagios \
 && chkconfig --level 2345 httpd on && make install-commandmode && make install-config && make install-webconf \
 ...
 && ./tools/setup && ./configure && make && make install && /usr/local/nagios/bin/nagios -v /usr/local/nagios/etc/nagios.cfg \
